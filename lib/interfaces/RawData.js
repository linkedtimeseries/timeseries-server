...
class RawData extends MultidimensionalInterface {
    ...
    constructor(config, commMan) {
        ...
        this._staticTriples = config.staticTriples != '' ? this.formatTriples('application/trig', Utils.getTriplesFromFile(config.staticTriples)[1], Utils.getTriplesFromFile(config.staticTriples)[0]) : '';
        this._byteCounter = 0;
    ...
    async createMetadata(fragment, index) {
        ...
        for (var i = 0; i < Object.keys(this._metadata).length; i++) {
            if (Object.keys(this._metadata)[i] != "@context") {
                quads.push({
                    subject: subject,
                    predicate: Object.keys(this._metadata)[i],
                    object: '"' + this._metadata[Object.keys(this._metadata)[i]]["@value"] + '"',
                    graph: '#Metadata'
                });
            }
        }
